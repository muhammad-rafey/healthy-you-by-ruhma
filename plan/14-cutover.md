...
- [ ] **Resend domain verified**: `hello@dietitianruhma.com` (or whichever sending subdomain we register) shows "Verified" in Resend dashboard, with SPF/DKIM/DMARC records published at the registrar. Send one real test from `/contact` form on the preview deploy and confirm it arrives in the inbox `hello@dietitianruhma.com` (or Dr. Ruhma's chosen inbox).
...
| --------------------------------- | ---------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Vercel Logs (project → Logs)      | 5xx errors, function errors, build errors      | Any 5xx → investigate immediately. Vercel sends email on hard failure; configure alerts to `hello@dietitianruhma.com`.                                                                             |
